...
public:
	...
	int CheckSignature(const unsigned char *data);
	int ReadChunk(unsigned &length,unsigned char *&buf,unsigned &chunkType,unsigned &crc,const unsigned char *data,int size,int &pos);
	int Decode(const unsigned char *data, int size);
