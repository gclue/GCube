...
void YsPngHeader::Decode(unsigned char dat[])
{
	...
	if(YsGenericPngDecoder::verboseMode==YSTRUE)
	{
		...
		LOGD("bitDepth=%d\n",bitDepth);
		LOGD("colorType=%d\n",colorType);
		LOGD("compressionMethod=%d\n",compressionMethod);
		LOGD("filterMethod=%d\n",filterMethod);
		LOGD("interlaceMethod=%d\n",interlaceMethod);
	}
}
...
int YsGenericPngDecoder::CheckSignature(const unsigned char *data)
{
	unsigned char buf[8];
	memcpy(buf, data, 8);
//	fread(buf,1,8,fp);
...
int YsGenericPngDecoder::ReadChunk(unsigned &length,unsigned char *&buf,unsigned &chunkType,unsigned &crc,const unsigned char *data,int size,int &pos)
{
...
int YsGenericPngDecoder::Decode(const unsigned char *data, int size)
{
	unsigned fileSize = size;

	if(CheckSignature(data)!=YSOK)
	{
		LOGE("The file does not have PNG signature.");
		return -1;
	}
	...
	while(ReadChunk(length,buf,chunkType,crc,data,size,pos)==YSOK && chunkType!=IEND)
	{
		switch(chunkType)
		{
		default:
